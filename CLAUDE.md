...
npm test           # vitest — client math (since/reweight/realwage/quiltRows/dcEscalation/dcMarkets/longLead) + csv/urlState/citation/dataFiles/dcAnchors/momentum/contribution/breadth
npm run e2e        # Playwright smoke + share + batch2-4 — 35 routes / 96 e2e tests, zero console errors
```
...
40 published files, each with a JSON Schema in `schemas/` validated inline as it lands:
...
- The gauge engine, nowcast, outlook, composites, DC cost index, geography panel, labor
  ...
  harness, long-lead board, rates panel, compute index, housing panel and the since-yesterday
  diff run in sixteen ISOLATED `try/except` blocks — a failure in any one still publishes
  status + qa (exit 0, visible on-site via `engine_ok` / `nowcast_ok` / `outlook_ok` /
  `composites_ok` / `datacenter_ok` / `geography_ok` / `labor_ok` / `commodities_ok` / `capacity_ok` / `markets_ok` / `grades_ok` / `longlead_ok` / `rates_ok` / `compute_ok` / `housing_ok` / `changes_ok`) instead of a hard crash or
  suppressing the other phases. `changes` runs LAST because it diffs every artifact just written.
