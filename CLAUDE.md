...
npm run e2e        # Playwright smoke + share + batch2 + batch3 — 31 routes / 86 e2e tests, zero console errors
```
...
36 published files, each with a JSON Schema in `schemas/` validated inline as it lands:
...
five long-lead packages' price legs).
...
- The gauge engine, nowcast, outlook, composites, DC cost index, geography panel, labor
  ...
  harness, and long-lead board run in twelve ISOLATED `try/except` blocks — a failure in any one still publishes
  status + qa (exit 0, visible on-site via `engine_ok` / `nowcast_ok` / `outlook_ok` /
  `composites_ok` / `datacenter_ok` / `geography_ok` / `labor_ok` / `commodities_ok` / `capacity_ok` / `markets_ok` / `grades_ok` / `longlead_ok`) instead of a hard crash or
