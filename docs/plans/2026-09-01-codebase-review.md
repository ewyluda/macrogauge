...
2. **No landing for the Project Controls audience**: nothing above the fold mentions data centers;
   ...
   direct Project Controls toolkit and renamed About navigation.**
...
5. **KPI tiles size to content** (orphaned 4th tile on `/capacity`, ragged mobile widths).
   → `repeat(auto-fit, minmax(220px, 1fr))`. **Complete on `feat/c1-c6-ui-pass`: all KPI rows
   now use the responsive auto-fit grid.**
6. **Header self-test pill is red for an advisory miss on every page.** → amber for advisory,
   ...
   status pills and failure-count labels reserve red for critical checks.**
...
4. C1–C6 — **complete on `feat/c1-c6-ui-pass` as a single UI-foundation changeset.**
   Verified with 876 pytest, 168 Vitest and 57 Playwright tests plus the 34-page static build.
